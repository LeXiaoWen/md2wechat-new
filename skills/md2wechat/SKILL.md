...
- Image generation may require extra provider config in `~/.config/md2wechat-new/config.yaml`.
...
- If the user asks to switch API provider/domain/model, change `api.text_provider`, `api.text_base_url`, `api.text_model`, or the matching `TEXT_*` environment variables.
