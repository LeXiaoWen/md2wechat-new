...
```bash
md2wechat-new humanize article.md --intensity gentle
md2wechat-new humanize article.md --intensity medium      # 默认，可省略
md2wechat-new humanize article.md --intensity aggressive
md2wechat-new humanize article.md --intensity authentic
```
...
```bash
md2wechat-new humanize article.md -i gentle
```
...
```bash
md2wechat-new humanize article.md -i aggressive
```
...
```bash
md2wechat-new humanize article.md -i authentic
```
...
```bash
md2wechat-new humanize article.md --show-changes
```
...
```bash
md2wechat-new humanize article.md -o article-humanized.md
```

同时看修改、输出到文件：

```bash
md2wechat-new humanize article.md --show-changes -o article-humanized.md
```
...
```bash
# 生成文章，默认 medium 强度
md2wechat-new write --style dan-koe --humanize article.md

# 指定强度
md2wechat-new write --style dan-koe --humanize=aggressive article.md
md2wechat-new write --style dan-koe --humanize=authentic article.md
```
...
```bash
md2wechat-new humanize article.md --json
```
...
> Agent 集成时的标准流程：`md2wechat-new humanize --json` → 解析 `prompt` → 调用 Claude → 把结果写到 `-o` 指定的文件。
...
```bash
cat article.md | md2wechat-new humanize -
pbpaste | md2wechat-new humanize - -o cleaned.md
```
...
更多命令参考 [USAGE.md](USAGE.md) 或执行 `md2wechat-new humanize --help`。
