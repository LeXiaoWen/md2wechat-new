...
```bash
sudo xattr -cr /Applications/md2wechat-new
```
...
API 模式需要你配置的文本 API Key，支持 OpenAI、DeepSeek、硅基流动和自定义 OpenAI-compatible 服务。
...
Save it to `~/.config/md2wechat-new/layout/custom/my-module.yaml`. The CLI picks it up on next run — no restart needed.
