...
`md2wechat-new` 提供了一组面向 Agent 和自动化脚本的发现命令，用来在执行前先确认当前 CLI 支持什么能力、有哪些可用资源、当前配置是否就绪。
...
1. `md2wechat-new capabilities --json`
2. `md2wechat-new providers list --json`
3. `md2wechat-new themes list --json`
4. `md2wechat-new prompts list --json`
...
md2wechat-new providers show openai --json
...
md2wechat-new themes show default --json
...
md2wechat-new prompts list --kind humanizer --json
md2wechat-new prompts list --kind refine --json
md2wechat-new prompts list --kind image --json
md2wechat-new prompts list --kind image --archetype cover --json
md2wechat-new prompts list --kind image --archetype infographic --json
...
```bash
md2wechat-new prompts show medium --kind humanizer --json
md2wechat-new prompts show authentic --kind humanizer --json
md2wechat-new prompts show default --kind refine --json
md2wechat-new prompts show cover-default --kind image --json
md2wechat-new prompts show cover-hero --kind image --archetype cover --tag hero --json
md2wechat-new prompts show infographic-victorian-engraving-banner --kind image --archetype infographic --tag victorian --json
```
...
```bash
md2wechat-new prompts render cover-default \
  --kind image \
...
```bash
md2wechat-new generate_image --preset cover-hero --article article.md
...
md2wechat-new generate_image --preset cover-hero --article article.md --model gemini-3-pro-image-preview
```
...
2. `./prompts`
3. `~/.config/md2wechat-new/prompts`
4. 内置 prompt 资产
...
md2wechat-new layout list --serves attention --json
md2wechat-new layout list --category opening --json
md2wechat-new layout list --tag brand --json
...
md2wechat-new layout render hero \
  --var eyebrow=深度观察 \
  --var title="公众号排版的真问题" \
  --json
...
md2wechat-new layout validate --stdin --json < article.md
```
...
2. `~/.config/md2wechat-new/layout/` — user-global overrides
