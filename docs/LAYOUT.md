...
**高级排版模块** = 一组预定义的视觉卡片组件，用 `:::模块名` 的语法写在 Markdown 里，由 md2wechat-new API 渲染成精准的微信 HTML。
...
md2wechat-new layout list --category sprint4 --json
```
...
```bash
md2wechat-new layout show hero --json | python3 -c "
import json,sys
...
```
:::image-steps
01 | 打开 md2wechat-new 配置文件 | https://example.com/step1.png
...
```
:::checklist[发布前检查]
md2wechat-new layout validate 通过 | done
...
```
:::resource-list
[{"icon":"🛠","name":"md2wechat-new CLI","url":"https://github.com/LeXiaoWen/md2wechat-new","desc":"Markdown 转微信的命令行工具"},{"icon":"📖","name":"Layout 教程","url":"https://github.com/LeXiaoWen/md2wechat-new/blob/main/docs/LAYOUT.md","desc":"本教程，43 个模块详解"}]
:::
```
...
03 | 用 render 生成 | md2wechat-new layout render <name> --var ...
04 | validate 校验 | md2wechat-new layout validate --file article.md --json
:::
...
# 4. 生成语法块
md2wechat-new layout render hero --var eyebrow=... --var title=... --json

# 5. 验证语法
md2wechat-new layout validate --file article.md --json

# 6. 转换
md2wechat-new convert article.md --output article.html
```
...
```bash
# 查看该模块的必填字段
md2wechat-new layout show <name> --json
...
```bash
# 查看正确的 JSON 结构
md2wechat-new layout show tweet --json
...
# 检查 API Key 是否已设置
md2wechat-new config show --format json | grep api_key
```
...
在 `~/.config/md2wechat-new/layout/<category>/<name>.yaml` 创建文件：
...
```bash
# 保存后，直接可用（无需重启）
md2wechat-new layout list --json | grep my-module
md2wechat-new layout show my-module --json
```
...
|--------|------|------|
| 1（最低） | 内置模块 | 二进制嵌入 |
| 2 | 用户全局 | `~/.config/md2wechat-new/layout/` |
| 3 | 项目本地 | `./layout/` |
