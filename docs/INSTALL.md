...
访问 [Releases](https://github.com/lexiaowenn/md2wechat-new/releases) 页面下载与你目标版本匹配的资产。
...
curl -LO https://github.com/lexiaowenn/md2wechat-new/releases/download/${VERSION}/${ASSET}
curl -LO https://github.com/lexiaowenn/md2wechat-new/releases/download/${VERSION}/checksums.txt
...
# 3. 移动到 PATH
sudo mv "${ASSET}" /usr/local/bin/md2wechat-new
...
```bash
git clone https://github.com/lexiaowenn/md2wechat-new.git
cd md2wechat-skill
go build -o md2wechat ./cmd/md2wechat-new
```
...
```bash
~/.local/bin/md2wechat-new version --json
~/.local/bin/md2wechat-new --help
```
...
```bash
rm $(go env GOPATH)/bin/md2wechat-new
```
...
```bash
# Linux/macOS
sudo rm /usr/local/bin/md2wechat-new
