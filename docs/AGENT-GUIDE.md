# md2wechat-new Agent 操作手册

本文档为 AI Agent 提供操作 md2wechat-new CLI 的完整指引，涵盖所有典型场景的命令序列。
...
2. `./themes` 本地目录
3. `~/.config/md2wechat-new/themes` 用户目录
4. 内置主题资产（38 个）

使用 `md2wechat-new themes show <theme_name> --json` 查看单个主题的详细配置。
...
使用 `md2wechat-new providers show <provider_name> --json` 查看单个 provider 需要的配置和支持的模型。
...
使用 `md2wechat-new prompts show <name> --kind <kind> --json` 查看单个 prompt 的具体内容和配置。

### 1.5 查看高级排版模块（43 个内置模块）

```bash
md2wechat-new layout list --json
md2wechat-new layout list --serves attention --json     # 按目标筛选
md2wechat-new layout list --serves readability --json
md2wechat-new layout list --serves memorability --json
md2wechat-new layout list --serves conversion --json
```
...
1. 配置文件 `~/.config/md2wechat-new/config.yaml`（或通过 `--config` 指定）
...
这会在 `~/.config/md2wechat-new/config.yaml` 创建示例配置文件。Agent 不应该自动调用此命令，除非用户明确要求重新初始化。
...
Brand Profile 是一个可选的品牌档案，位于 `~/.config/md2wechat-new/brand.md`，由 Agent 读取但不被 CLI 解析。
...
brand_path = os.path.expanduser("~/.config/md2wechat-new/brand.md")
brand_content = ""
...
```bash
result=$(md2wechat-new brand show --json)
brand_content=$(echo "$result" | jq -r '.data.content')
```

### 3.3 Brand Profile 引导流程（如果不存在）

当 `md2wechat-new brand show` 返回 `BRAND_NOT_FOUND` 时，Agent 应发起 3 问引导：
...
根据用户回答，创建或编辑 `~/.config/md2wechat-new/brand.md`。
...
```bash
md2wechat-new layout validate --file article.md --json
```
...
```bash
# 查看模块完整规范
md2wechat-new layout show hero --json
md2wechat-new layout show callout --json

# 查看渲染示例（需要 API 服务）
md2wechat-new layout render callout --var CONTENT="重要提示" --var LEVEL="warning"
```
...
```bash
md2wechat-new convert article.md --output output.html
```
...
```bash
md2wechat-new convert article.md --mode ai --output output.html
```
...
```bash
md2wechat-new convert article.md --theme bytedance --output output.html
md2wechat-new convert article.md --theme minimal-blue --output output.html
md2wechat-new convert article.md --theme autumn-warm --output output.html
```
...
md2wechat-new preview article.md --output /path/to/preview.html
```
...
```bash
md2wechat-new inspect article.md --upload --json
```
...
```bash
md2wechat-new generate_cover "article title" --output cover.jpg
md2wechat-new upload_image cover.jpg --json
```
...
```bash
md2wechat-new convert article.md --draft --cover ./cover.jpg --output draft.html
md2wechat-new create_draft draft.html --json
```
...
```bash
md2wechat-new create_image_post article.md --cover ./cover.jpg --json
```
...
```bash
md2wechat-new layout show hero --json
md2wechat-new layout show verdict --json
md2wechat-new layout show callout --json
```
...
```bash
md2wechat-new layout render callout --var CONTENT="测试内容" --var LEVEL="info"
md2wechat-new layout render cta --var TITLE="立即订阅" --var ACTION="subscribe"
```
...
**症状**：`md2wechat-new brand show` 返回 `BRAND_NOT_FOUND`（当命令可用时）
...
根据用户回答，创建或编辑 `~/.config/md2wechat-new/brand.md`。
# 或等待 md2wechat-new brand init 命令上线
```

### 8.3 排版语法错误

**症状**：`md2wechat-new layout validate --file article.md --json` 返回错误

**处理**：
```bash
# 1. 查看具体错误
md2wechat-new layout validate --file article.md --json | jq '.data.errors'

# 2. 查看对应模块的正确语法
md2wechat-new layout show <failing_module_name> --json

# 3. 查看实际渲染示例
md2wechat-new layout render <module_name> --var KEY=VALUE
...
3. 环境变量（如 CONVERT_MODE=ai）
    ↓
4. 配置文件（~/.config/md2wechat-new/config.yaml）
    ↓
...
# 2. 检查并初始化配置
md2wechat-new config show --format json
# 如果缺少关键配置，提示用户运行 md2wechat-new config init

# 3. 发起 3 问流程创建 Brand Profile（当命令可用时）
# 或手动创建 ~/.config/md2wechat-new/brand.md
...
# 7. 检查发布就绪
md2wechat-new inspect article.md --draft --cover ./cover.jpg --json
...
# 2. 查看失败模块的规范
md2wechat-new layout show <failing_module> --json

# 3. 查看模块渲染示例
md2wechat-new layout render <failing_module> --var KEY=VALUE
...
# 6. 转换并预览
md2wechat-new convert article.md --mode api --output output.html
md2wechat-new preview article.md --output preview.html
```
...
| 目标 | 命令 |
|------|------|
| 查看当前能力 | `md2wechat-new capabilities --json` |
| 查看可用主题 | `md2wechat-new themes list --json` |
| 查看可用 Provider | `md2wechat-new providers list --json` |
| 查看 Prompt 模板 | `md2wechat-new prompts list --json` |
| 查看排版模块 | `md2wechat-new layout list --json` |
| 验证排版语法 | `md2wechat-new layout validate --file article.md --json` |
| 查看配置 | `md2wechat-new config show --format json` |
| 验证配置 | `md2wechat-new config validate` |
| 转换文章（API） | `md2wechat-new convert article.md --output output.html` |
| 转换文章（AI） | `md2wechat-new convert article.md --mode ai --output output.html` |
| 检查发布就绪 | `md2wechat-new inspect article.md --draft --json` |
| 预览文章 | `md2wechat-new preview article.md` |
| 创建草稿 | `md2wechat-new convert article.md --draft --cover ./cover.jpg` |
| 查看模块规范 | `md2wechat-new layout show hero --json` |
| 测试模块渲染 | `md2wechat-new layout render callout --var CONTENT="test" --var LEVEL="info"` |
...
```bash
md2wechat-new convert article.md --json
md2wechat-new inspect article.md --json
md2wechat-new layout validate --file article.md --json
```
...
*最后更新：与 md2wechat-new v0.1.0 同步*
*下一计划版本 (v0.2.0)：md2wechat-new brand 命令*
