...
使用你配置的 OpenAI-compatible 文本 API 进行转换，支持 OpenAI、DeepSeek、硅基流动和自定义中转。

```bash
md2wechat convert article.md --mode api --api-key "your_text_api_key"
```

**特点**：
- 转换速度快
- 结果稳定一致
- 需要配置文本 API Key、provider 和模型
...
生成排版 prompt，由外部 AI/Agent 完成 HTML，更加灵活。
...
# 编辑 ~/.config/md2wechat-new/config.yaml，填入微信 AppID、Secret 和文本 API Key
