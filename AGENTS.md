...
10. If the task publishes a new npm version, manually trigger `npx cnpm sync @lexiaowenn/md2wechat-new` after npm publish so `npmmirror` users do not get a stale tarball 404 on the fresh release.
...
./md2wechat-new convert <layout-test.md> --mode api --output /tmp/layout-smoke.html
...
./md2wechat-new layout validate --file <layout-test.md> --json
```
...
1. `gofmt -l .`
2. `GOCACHE=/tmp/md2wechat-go-build go test ./internal/promptcatalog ./cmd/md2wechat-new`
...
The Brand Profile lives at `~/.config/md2wechat-new/brand.md` (Markdown format, not YAML). Initialize with `md2wechat brand init`. Agents read it for style context; the CLI does not parse it. Never commit brand.md to the repository.
