...
func TestValidateConvertConfigRequiresAPIKeyInAPIMode(t *testing.T) {
	...
	if err := validateConvertConfig(); err == nil || !strings.Contains(err.Error(), "TEXT_API_KEY") {
		t.Fatalf("validateConvertConfig() error = %v", err)
	}
}
...
func TestCreateWeChatDraftRequiresCoverImage(t *testing.T) {
	...
	cfg = &config.Config{WechatAppID: "appid", WechatSecret: "secret", TextAPIKey: "api-key"}
	log = zap.NewNop()
	convertMode = "api"
