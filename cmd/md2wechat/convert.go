...
Supports two conversion modes:
  - api: Use configured text API provider (OpenAI-compatible, requires API key)
...
func init() {
	...
	convertCmd.Flags().StringVar(&convertAPIKey, "api-key", "", "API key for the configured text provider")
...
func validateConvertConfig() error {
	...
	if convertMode == "api" {
		if convertAPIKey == "" && strings.TrimSpace(cfg.TextAPIKey) == "" {
			return newCLIError(codeConvertInvalid, "TEXT_API_KEY is required for API mode")
		}
	}
