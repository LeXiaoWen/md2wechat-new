...
Config file search order:
  1. ~/.config/md2wechat-new/config.yaml  (global config, recommended)
  2. ~/.md2wechat.yaml                (global config)
  3. ./md2wechat-new.yaml                  (project config)
...
This is the global config location, used by all your projects.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 {
				...
				configDir := homeDir + "/.config/md2wechat-new"
...
func printYAMLConfig(cfg *config.Config, maskSecret bool) {
	...
	fmt.Println("api:")
	fmt.Printf("  text_provider: %s\n", cfg.TextProvider)
	fmt.Printf("  text_key: %s\n", maskAPIKey(cfg.TextAPIKey, maskSecret))
	fmt.Printf("  text_base_url: %s\n", cfg.TextAPIBase)
	fmt.Printf("  text_model: %s\n", cfg.TextModel)
	fmt.Printf("  text_temperature: %.2f\n", cfg.TextTemperature)
...
func maskAPIKey(key string, mask bool) string {
	if !mask || key == "" || key == "your_md2wechat_api_key" || key == "your_text_api_key" || key == "your_image_api_key" {
		return key
	}
	if len(key) <= 8 {
		return "***"
