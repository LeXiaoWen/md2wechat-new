...
func TestConvertViaAPIStripsFrontMatterBeforeSendingMarkdown(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte("{\"choices\":[{\"message\":{\"role\":\"assistant\",\"content\":\"```html\\n<p>ok</p>\\n```\"}}]}"))
	}))
	defer server.Close()
	...
	userPrompt := received.Messages[1].Content
	if strings.Contains(userPrompt, "title: Frontmatter Title") {
		t.Fatalf("api request still contains frontmatter: %q", userPrompt)
	}
	if !strings.Contains(userPrompt, "# Heading Title") || !strings.Contains(userPrompt, "body") {
		t.Fatalf("api request missing body content: %q", userPrompt)
	}
}
