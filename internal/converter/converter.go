...
func (c *converter) validateRequest(req *ConvertRequest) error {
	...
	switch req.Mode {
	case ModeAPI:
		if req.APIKey == "" && strings.TrimSpace(c.cfg.TextAPIKey) == "" {
			return ErrMissingAPIKey
		}
		if req.APIKey == "" {
			req.APIKey = c.cfg.TextAPIKey
		}
	case ModeAI:
		// AI 模式不需要额外验证
	}
...
var (
	...
	ErrMissingAPIKey = &ConvertError{Code: "MISSING_API_KEY", Message: "TEXT_API_KEY is required for API mode"}
