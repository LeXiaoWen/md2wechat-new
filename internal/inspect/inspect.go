...
func buildChecks(input *Input, result *Result) []Check {
	...
	if result.Context.Mode == "api" && !apiKeyAvailable(input.Config) {
		checks = append(checks, Check{
			...
			Message:      "API mode requires TEXT_API_KEY",
			Field:        "mode",
			SuggestedFix: "set TEXT_API_KEY or switch to --mode ai",
		})
	}
	...
	if input.UploadRequested || input.DraftRequested {
		if !uploadReadyFor(input.Config) {
			checks = append(checks, Check{
				...
				Field:        "config",
				SuggestedFix: "set WECHAT_APPID and WECHAT_SECRET or configure them in ~/.config/md2wechat-new/config.yaml",
			})
		}
	}
...
func apiKeyAvailable(cfg *config.Config) bool {
	return cfg != nil && strings.TrimSpace(cfg.TextAPIKey) != ""
}
