...
class Md2wechat < Formula
  ...
  on_macos do
    on_arm do
      url "${base_url}/md2wechat-new_Darwin_arm64.tar.gz"
      sha256 "${darwin_arm64_sha}"
    end
    on_intel do
      url "${base_url}/md2wechat-new_Darwin_x86_64.tar.gz"
  ...
  on_linux do
    on_arm do
      url "${base_url}/md2wechat-new_Linux_arm64.tar.gz"
      sha256 "${linux_arm64_sha}"
    end
    on_intel do
      url "${base_url}/md2wechat-new_Linux_x86_64.tar.gz"
  ...
  test do
    ...
    payload = JSON.parse(shell_output("#{bin}/md2wechat-new version --json"))
