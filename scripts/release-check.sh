...
! grep -q 'HOMEBREW_TAP_GITHUB_TOKEN' .github/workflows/release.yml || fail "release workflow must not require a Homebrew tap token for the fork release"
...
grep -q 'npm install -g @lexiaowen/md2wechat-new' README.md || fail "README must document the npm install path"
grep -q 'npm install -g @lexiaowen/md2wechat-new' docs/INSTALL.md || fail "docs/INSTALL.md must document the npm install path"
grep -q 'npx cnpm sync @lexiaowen/md2wechat-new' docs/INSTALL.md || fail "docs/INSTALL.md must document the post-publish npmmirror sync step"
grep -q 'npx cnpm sync @lexiaowen/md2wechat-new' docs/FAQ.md || fail "docs/FAQ.md must document the npmmirror sync recovery path"
grep -q 'brew install lexiaowenn/tap/md2wechat-new' README.md || fail "README must document the tap install path"
grep -q 'brew install lexiaowenn/tap/md2wechat-new' docs/INSTALL.md || fail "docs/INSTALL.md must document the tap install path"
! grep -q 'raw.githubusercontent.com/LeXiaoWen/md2wechat-new/main/scripts/install.sh' README.md || fail "README must not point the md2wechat installer at main"
! grep -q 'raw.githubusercontent.com/LeXiaoWen/md2wechat-new/main/scripts/install.ps1' README.md || fail "README must not point the md2wechat installer at main"
! grep -q 'raw.githubusercontent.com/LeXiaoWen/md2wechat-new/main/scripts/install-openclaw.sh' README.md || fail "README must not point the OpenClaw installer at main"
! grep -q 'raw.githubusercontent.com/LeXiaoWen/md2wechat-new/main/scripts/install.sh' docs/INSTALL.md || fail "docs/INSTALL.md must not point the md2wechat installer at main"
! grep -q 'raw.githubusercontent.com/LeXiaoWen/md2wechat-new/main/scripts/install.ps1' docs/INSTALL.md || fail "docs/INSTALL.md must not point the md2wechat installer at main"
! grep -q 'raw.githubusercontent.com/LeXiaoWen/md2wechat-new/main/scripts/install-openclaw.sh' docs/OPENCLAW.md || fail "docs/OPENCLAW.md must not point the OpenClaw installer at main"
grep -q 'md2wechat-new inspect article.md' README.md || fail "README must document inspect as part of the confirm-first flow"
grep -q 'md2wechat-new preview article.md' README.md || fail "README must document preview as part of the confirm-first flow"
grep -q 'md2wechat-new inspect article.md' docs/QUICKSTART.md || fail "docs/QUICKSTART.md must document inspect"
grep -q 'md2wechat-new preview article.md' docs/QUICKSTART.md || fail "docs/QUICKSTART.md must document preview"
grep -q 'md2wechat-new inspect article.md --json' docs/DISCOVERY.md || fail "docs/DISCOVERY.md must document inspect --json"
grep -q 'md2wechat-new preview article.md --json' docs/DISCOVERY.md || fail "docs/DISCOVERY.md must document preview --json"
grep -q 'md2wechat-new inspect article.md' docs/FAQ.md || fail "docs/FAQ.md must document inspect"
grep -q 'md2wechat-new preview article.md' docs/FAQ.md || fail "docs/FAQ.md must document preview"
grep -q 'md2wechat-new inspect article.md' docs/USAGE.md || fail "docs/USAGE.md must document inspect"
grep -q 'md2wechat-new preview article.md' docs/USAGE.md || fail "docs/USAGE.md must document preview"
