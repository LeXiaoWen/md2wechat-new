...
metadata: {"clawdbot":{"emoji":"📝","requires":{"bins":["md2wechat"],"env":["WECHAT_APPID","WECHAT_SECRET"]},"install":[{"id":"brew","kind":"brew","formula":"lexiaowenn/tap/md2wechat-new","bins":["md2wechat"],"label":"Install md2wechat (brew)"},{"id":"go","kind":"go","module":"github.com/lexiaowenn/md2wechat-new/cmd/md2wechat@latest","bins":["md2wechat"],"label":"Install md2wechat (go)"}]}}
---
...
- Image generation may require additional image-service configuration in `~/.config/md2wechat-new/config.yaml`.
...
- Check configuration in this order:
  ...
  2. environment variables such as `TEXT_PROVIDER`, `TEXT_API_KEY`, `TEXT_API_BASE`, and `TEXT_MODEL`
  3. project-local `md2wechat.yaml`, `md2wechat.yml`, or `md2wechat.json`
- If the user asks to switch API provider/domain/model, update `api.text_provider`, `api.text_base_url`, `api.text_model`, or the matching `TEXT_*` environment variables.
...
```bash
test -f ~/.config/md2wechat-new/brand.md   && cat ~/.config/md2wechat-new/brand.md   || echo "no brand profile — using system defaults"
```
...
品牌档案存储在 `~/.config/md2wechat-new/brand.md`（可选文件，用户创建）。
...
```bash
test -f ~/.config/md2wechat-new/brand.md   && cat ~/.config/md2wechat-new/brand.md   || echo "no brand profile"
```
...
```bash
mkdir -p ~/.config/md2wechat-new
cat > ~/.config/md2wechat-new/brand.md << 'BRAND_EOF'
schema_version: 1
name: [Q1答案]
...
```bash
cat ~/.config/md2wechat-new/brand.md

python3 -c "
content = open('$HOME/.config/md2wechat-new/brand.md').read()
